...
-- Ya existía (respaldo del cierre diario); se deja igual, solo con
-- "if not exists" por si se corre este script en una base nueva.
...
-- Mismo criterio que ya usa `cierres`: la app usa la clave anon
-- (publishable) desde el cliente, así que el control de acceso real
-- vive aquí, no en el secreto de la clave. Como es un solo restaurante
-- sin login todavía, se da acceso completo al rol anon en estas 3
-- tablas nuevas. El día que haya autenticación de meseros/admin, estas
-- policies son el punto exacto donde se restringe por rol.
...
-- ── 3. REALTIME ────────────────────────────────────────────
-- Necesario para que un cambio hecho en un dispositivo (ej. abrir una
-- mesa desde el iPad de la mesera) se refleje al instante en los demás
-- (ej. la pantalla de caja), sin recargar la página.
...
-- Opcional: la app ya siembra estos mismos datos sola la primera vez
-- que se conecta y encuentra las tablas vacías (ver `sembrarProductos`
-- / `sembrarMesas` en index.html). Correr esto a mano sirve para tener
-- los datos listos ANTES de abrir la app por primera vez, o para
-- restaurar el catálogo original si se necesita.
